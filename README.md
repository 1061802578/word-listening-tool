...
- 支持英语、日语和自动识别三种语言模式。
- 默认使用 Tesseract 完整识别，避免部署到 HTTPS 后浏览器自带识别只返回部分文本。
...
6. 识别引擎建议保持“Tesseract 完整识别”。部署到 HTTPS 后，部分浏览器的自带识别可能速度快但只返回部分文本。

7. 图片比较糊、小字多或识别乱码时，图片增强保持“强力增强”。

8. 文本比较整齐时点击“普通导入”；文本比较乱或需要补释义、音标/假名时，点击“DeepSeek 整理导入”。

9. 在词条列表里确认、编辑、筛选和勾选需要的内容。
